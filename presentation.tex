...
\author[Margaret A. Barker]{Margaret A. Barker}
...
\begin{document}
    ...
    \begin{frame}{VSEPR theory}
        Molecular shape is determined by Valence Shell Electron Pair Repulsion theory (VSEPR). The name of the theory explains most of what you need to know:
        \vs
    ...
    \begin{frame}{VSEPR Theory \cite{chemlearner}}
        \begin{center}
            \includegraphics[width=3in]{figures/vsepr.jpg}
        \end{center}
    \end{frame}
    ...
    \begin{frame}{Point groups of low symmetry}
        ...
        \begin{center}\includegraphics[width=0.75in]{molecules/c1example.jpg}\end{center}
        ...
        \onslide<2->{
            ...
            \begin{center}\includegraphics[width=0.75in]{molecules/csexample.jpg}\end{center}
        }

        \onslide<3>{
            Molecules in the $C_i$ group have only an inversion center.

            \begin{center}\includegraphics[width=0.75in]{molecules/ciexample.jpg}\end{center}
        }
    \end{frame}
    ...
    \begin{frame}{Point groups of high symmetry}
        ...
        \begin{center}\includegraphics[width=0.75in]{molecules/cinfvexample.jpg}\end{center}
        ...
        \onslide<2->{
            ...
            \begin{center}\includegraphics[width=0.75in]{molecules/dinfhexample.jpg}\end{center}
        }
        ...
        \onslide<3>{
            ...
            \begin{center}\includegraphics[width=0.75in]{molecules/tdexample.jpg}\end{center}
        }
    \end{frame}
    ...
    \begin{frame}{Point groups of high symmetry}
        ...
        \begin{center}\includegraphics[width=0.75in]{molecules/ohexample.jpg}\end{center}
        ...
        \onslide<2>{
            ...
            \begin{center}\includegraphics[width=0.75in]{molecules/bucky.png}\end{center}
        }
    \end{frame}

    \begin{frame}{Other point groups}
        \begin{center}
            \includegraphics[width=3.75in]{figures/flowchart.png}
        \end{center}
    \end{frame}
    ...
    \begin{frame}{Other point groups}
        ...
        \begin{center}\includegraphics[width=0.75in]{molecules/c2hexample1.jpg}\includegraphics[width=0.75in]{molecules/c2hexample2.jpg}\includegraphics[width=0.75in]{molecules/c3hexample.jpg}\end{center}
        ...
        \onslide<2>{
            ...
            \begin{center}\includegraphics[width=0.75in]{molecules/c2vexample.jpg}\includegraphics[width=0.75in]{molecules/c3vexample.jpg}\includegraphics[width=0.75in]{molecules/c4vexample.jpg}\end{center}
        }
    \end{frame}
    ...
    \begin{frame}{Other point groups}
        ...
        \begin{center}\includegraphics[width=0.75in]{molecules/c2example.jpg}\end{center}
        ...
        \onslide<2>{
            ...
            \begin{center}\includegraphics[width=0.75in]{molecules/d3hexample1.jpg}\includegraphics[width=0.75in]{molecules/d3hexample2.jpg}\includegraphics[width=0.75in]{molecules/d4hexample.jpg}\includegraphics[width=0.75in]{molecules/d6hexample.jpg}\end{center}
        }
    \end{frame}
    ...
    \begin{frame}{Other point groups}
        ...
        \begin{center}\includegraphics[width=0.75in]{molecules/d3dexample.jpg}\end{center}
    ...
    \begin{frame}{Matrix representation of $C_{2v}$}
        ...
        \begin{center}\includegraphics[width=1.5in]{axes/waterxyz.png}\end{center}
    \end{frame}
    ...
    \begin{frame}{Matrix representation of $C_{3v}$}
        ...
        \begin{center}\includegraphics[width=1.5in]{axes/ammoniaxyz.png}\end{center}
    \end{frame}
    ...
    \begin{frame}{Character tables}
        Character tables provide all of the symmetry information in a given point group in a succinct way. In a sense, they prepackage the underlying group theory into an easily read table that is shared by every molecule in a given point group. We will look at the construction of some simple character tables, and then we will explore some of their applications.
        \vs
    ...
    \begin{frame}{Character tables}
        ...
        We now have a complete character table for $C_{2v}$. Additional information has been added to the last two columns.
    \end{frame}
    ...
    \begin{frame}{Character tables}
        ...
        $R_x$, $R_y$, and $R_z$ indicate the rotational axes that are symmetric with respect to the irreps. For example, $C_2$ corresponds to $R_z$. We have that $C_2$ is symmetric with respect to $A_2$, so $R_z$ is symmetric with respect to $A_2$. $\sigma_{v1}$ corresponds to a reflection over the $xz$ plane, which is normal to the $y$ axis. We have that $\sigma_{v1}$ is symmetric with respect to $B_1$, so $R_y$ is symmetric with respect to $B_1$. Similarly, $R_x$ is symmetric with respect to $B_2$.
    \end{frame}
    ...
    \begin{frame}{Character tables}
        ...
        $x^2$, $y^2$, $xy$, etc. represent math functions that are symmetric with respect to the irreps. For instance, we know that $xy$ is positive in quadrants I and III and negative in II and IV, and a $180^\circ$ rotation about the $z$ axis maintains these signs. However, a $180^\circ$ rotation about the $x$ or $y$ axes reverses these signs. Hence, $xy$ appears in the same row as $R_z$ and is symmetric with respect to $A_2$. The functions $x^2$, $y^2$, and $z^2$ are always positive, so they are symmetric with respect to $A_1$.
    \end{frame}
    ...
    \begin{frame}{Expressing reducible representations in terms of irreps}
        \begin{proof}
            ...
            Multiplying both sides by $n_j$, and taking the sum over all $j$ irreps, we have \begin{equation}\sum_C n_C\sum_jn_j \chi_i(C)\chi_j(C)=h\sum_jn_j\delta_{ij}=hn_i.\label{lemmastep2}\end{equation}
    ...
    \begin{frame}{Molecular vibrations}
        Individual atoms can move around slightly within a molecule, producing molecular vibrations. To study these vibrations, we first assign a separate 3D coordinate axis to each atom.
        \vs
        
        For a molecule with $N$ atoms, each atom can move independently in three dimensions, so there are $3N$ possible motions, called \emph{degrees of freedom}.
        \vs
    ...
    \begin{frame}{Molecular vibrations}
        As we will see, certain vibrational modes are represented by peaks in IR and Raman spectra. Counting and classifying vibrational modes is essential, as it lets chemists predict IR and Raman spectra from molecular structure. As we will see, character tables provide a systematic way to make these predictions.
        \vs
        
        If we create a $3N\times3N$ matrix to represent the effect of a symmetry operation on the position of each atom, an element in the diagonal of the matrix will be 1 if the atom remains in its original location, -1 if it reverses direction, and 0 if it moves. We can then take the character of the matrix for each symmetry operation. Since operations in the same class have the same character, we only need to construct one such matrix for each class.
        \vs
        
        However, constructing these matrices explicitly is tedious. Thankfully, the only thing we need from these matrices is their characters, and we can find the characters without explicitly constructing the matrices.
    \end{frame}

    \begin{frame}{Molecular vibrations}
        We will use the vibrations of water as an example. For convenience, we stick with the same coordinate axes that were used earlier. This time, however, each atom gets its own set of coordinate axes, and each atom is considered to be at its own origin. Since water has three atoms, it has nine degrees of freedom.
        \vs
    
        Since water is nonlinear, it has $3N-6=3$ vibrational modes. These are \emph{symmetric stretch}, \emph{antisymmetric stretch}, and \emph{symmetric bend}. Each of these vibrational modes changes the \emph{dipole moment} of water, causing its polarity to oscillate slightly.
        \vs
        
        Demo: \url{https://www.chem.purdue.edu/jmol/vibs/h2o.html}
    \end{frame}

    \begin{frame}{Molecular vibrations}
        We could create a $9\times 9$ matrix for each of the four symmetry classes in water. However, we can find the characters of each matrix without constructing the matrices. In $E$, every atom remains in the same position, so $\chi_E=9$. In $C_2$, the hydrogen atoms both move, so they contribute nothing to the character. The oxygen atom reverses direction with respect to $x$ $(-1)$ and $y$ $(-1)$, but not with respect to $z$ $(1)$. Thus, oxygen contributes a net of $-1$ to the character. Hence $\chi_{C_2}=-1$. In $\sigma_{v1}~(xz)$, in the plane of the molecule, the $x$ and $z$ directions stay the same for each atom, while the $y$ direction flips for each atom. Then $\chi_{\sigma_{v1}}=3-3+3=3$. In the $\sigma_{v2}~(yz)$, the hydrogen atoms swap positions, so they contribute nothing to the character. The oxygen atom reverses direction with respect to $x$ $(-1)$ but not with respect to $y$ $(1)$ or $z$ $(1)$. Therefore, $\chi_{\sigma_{v2}}=1$.
    \end{frame}
    
    \begin{frame}{Molecular vibrations}
        We are now ready to show the full $C_{2v}$ character table again, this time with $\Gamma$ included.
    ...
    \begin{frame}{Molecular vibrations}
        We can now use Lemma 1 to express $\Gamma$ as a linear combination of the irreps $A_1, A_2, B_1$, and $B_2$. We have \[n_{A_1}=\frac14((9)(1)+(-1)(1)+(3)(1)+(1)(1))=3.\]
        Similarly, we calculate $n_{A_2}=1$, $n_{B_1}=3$, and $n_{B_2}=2$. We find that \begin{equation}\Gamma = 3A_1+A_2+3B_1+2B_2.\label{linearcombo}\end{equation}
        The three translational modes of $C_{2v}$ correspond to the irreps that have $x$, $y$, or $z$ listed on the right hand side of the character table, so the three translational modes correspond to $A_1$, $B_1$, and $B_2$. The three rotational modes correspond to the irreps that have $R_x$, $R_y$ and $R_z$ listed, which are $A_2$, $B_1$, and $B_2$.
    \end{frame}

    \begin{frame}{Molecular vibrations}
        Taking \eqref{linearcombo} and subtracting $A_1 + B_1 + B_2 + A_2 + B_1 + B_2$, for the translational and rotational modes, we have $2A_1+B_1$ remaining, which are the three vibrational modes. Two correspond to $A_1$ and one corresponds to $B_1$.
        \vs
    ...
    \begin{frame}{Applications: Spectroscopy}
        ...
        \begin{center}
            \includegraphics[width=4.5in]{figures/irwater.png}
        \end{center}
    \end{frame}
    ...
    \begin{frame}{Applications: Chirality \cite{monash}}
        ...
        \begin{center}\includegraphics[width=2.5in]{figures/enantiomers.png}\end{center}
    ...
    \begin{frame}{Applications: Chirality \cite{halpern}}
        ...
        \begin{center}
            \includegraphics[width=3in]{figures/chiral.jpg}
        \end{center}
    \end{frame}
    ...
    \begin{frame}{Applications: Chirality}
        ...
        If a molecule contains any $\sigma$, $S_n$, or $i$ symmetry elements, it is achiral. Hence, the only symmetries possible for a chiral compound are $E$ ane $C_n$. A look at the character table of a molecule's point group will tell which symmetry elements are present, from which one can deduce that the molecule is chiral or achiral.
        \vs
        
        For instance, by looking at the character tables for $C_{2v}$ and $C_{3v}$, we see the presence of $\sigma$ symmetry elements, precluding molecules that belong to these point groups from being chiral. Thus, water and ammonia are achiral molecules.
    \end{frame}
